...
<div class="container container">
    ...
    <div class="m-top m-bottom">
        ...
        <form action="{{ route('subject.update', $subject->id) }}" method="POST" enctype="multipart/form-data">
            ...
            @method('PUT')
            <div class="mb-3">
                <label for="" class="add-label">Название тематики</label>
                <input type="text" name="name" class="form-control" value="{{ $subject->name }}">
            </div>
            <div class="mb-3">
                <label for="" class="add-label">Положительный результат</label>
                <input type="text" name="true_position" class="form-control" value="{{ $subject->true_position }}">
            </div>
            <div class="mb-3">
                <label for="" class="add-label">Отрицательный результат</label>
                <input type="text" name="false_position" class="form-control" value="{{ $subject->false_position }}">
            </div>
            <div class="row">
                <div class="col-md-6">
