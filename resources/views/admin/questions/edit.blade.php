...
<div class="container container">
    ...
    <div class="m-top m-bottom">
        ...
        <form action="{{ route('question.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="mb-3">
                <input type="text" name="subject_id" value="{{ $subject->id }}" class="form-control" hidden placeholder="{{ $subject->name }}">
            </div>
            <div class="mb-3">
                <label for="" class="add-label">Введите вопрос</label>
                <input type="text" name="question_text" class="form-control">
            </div>
            <div class="row">
                <div class="col-md-6">
                    <button type="submit" class="button">Сохранить</button>
                    <a href="{{ route('option.create') }}">Добавить ответы</a>
                </div>
            </div>
        </form>
    </div>
