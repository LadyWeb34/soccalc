...
<div class="container container">
    ...
    <div class="m-top m-bottom">
        ...
        <form action="{{ route('option.update',$option->id) }}" method="post" enctype="multipart/form-data">
            ...
            @method('PUT')
            <div class="mb-3">
                <label for="" class="add-label">Выберите вопрос</label>
                <select name="question_id" class="form-select" aria-label="-">
                    @if ($option)
                            <option value="{{ $option->question_id }}">{{ $option->question->question_text }}</option>
                    @endif
                  </select>
            </div>
            <div class="mb-3">
                <label for="" class="add-label">Введите вариант ответ</label>
                <input type="text" name="option_text" value="{{ $option->option_text }}" class="form-control">
            </div>
            <div class="mb-3">
                <label for="" class="add-label">Кол-во баллов при позитивном решении</label>
                <input type="number" name="points" value="{{ $option->points }}" class="form-control">
            </div>
            <div class="row">
                <div class="col-md-6">
